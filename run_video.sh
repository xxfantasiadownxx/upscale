#!/bin/bash
# run_video.sh — Batch upscale animated videos to 1080p using realesr-general-x4v3
# Usage: ./run_video.sh
...
for i in "${!VIDEO_FILES[@]}"; do
  ...
  OUTPUT_FILE="${FILENAME%.*}_video_1080p.mkv"
...
")
  ...
  touch "$POLL_SENTINEL"
  poll_stage "$COMPOSE_LOG" "video-ffmpeg-extract" "video-upscale" "$i" "$FILENAME" "$POLL_SENTINEL" &
  POLL_PID=$!

  run_compose "$BASE_DIR/docker-compose.yml" "upscale-video" "$COMPOSE_LOG"
  STAGE1_EXIT=$?
  ...
  run_compose "$BASE_DIR/docker-compose.reassemble.yml" "upscale-video-reassemble" "$REASSEMBLE_LOG"
  STAGE2_EXIT=$?
